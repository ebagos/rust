...
let data: [f64; 100] = [
    ...
    println!("average = {}", avrg(&data));
}
/*
    cloned()を入れると動作した！
*/
fn freq(data: &[f64; 100], min: f64, d: f64) -> usize {
    data
    .iter()
    .cloned()
    .filter(|&x| x > min && x <= min + d)
    .count()
}

// sum()はOrdでないと使えなくてf32とかf64はOrdではない！
fn sum(data: &[f64]) -> f64 {
    data.iter().fold(0.0, |sum, &a| sum + a)
//    data.iter().fold(0.0, |sum, a| sum + a)
//    見かけ上同じ動作に見えるので理解が必要！
}

fn avrg(data: &[f64]) -> f64 {
    sum(data) / data.iter().count() as f64
}